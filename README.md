...
The `process.node.id` property is required when using the production profile. This property ensures that the **short
URLs** generated by the service are unique across different instances or nodes of the application. By providing a unique
node ID for each instance, the system can generate distinct and non-conflicting shortened URLs even when the application
is deployed in a distributed environment.

### Configuration Methods

You can define the `process.node.id` property in one of the following ways:

1. When Running Locally:
   If you're running the application locally (without Docker), define the `process.node.id` in the `application.yml`
   file:

    ```yml
    process:
      node:
        id: 1  # Specify the node ID as an integer (ensure it's unique per instance)
    ```
   Replace `1` with any unique integer for each local instance of your service.

2. When Running in a Docker Container:
   If you're running the application in a Docker container, set the `process.node.id` property dynamically using an
   environment variable. This is particularly useful when deploying the application in containerized or cloud
   environments where each instance needs a different node ID.

    ```bash
      docker run -p 9090:9090 \
       -e PROCESS_NODE_ID=2318 \
       -e SPRING_PROFILES_ACTIVE=prod \
       akgarg0472/urlshortener-service:tag
    ```

   In this example, the environment variable `PROCESS_NODE_ID=1` is set to `1`, but you can replace it with any
   unique integer value for each container instance.

#### Why is `process.node.id` Important?

The `process.node.id` property ensures that the short URLs generated by each instance are unique. For example, when
multiple instances of the application are running behind a load balancer, this property helps in generating distinct URL
shorteners across instances, preventing any conflicts.

For instance, if two instances are running, one with `process.node.id=1` and another with `process.node.id=2`, the
generated short URLs will be distinct, ensuring that the same URL doesn’t get shortened to the same code in both
instances.
